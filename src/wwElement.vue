    ...
    >
      ...
      >
        ...
        <div class="tokban-modal-body">
          ...
          <!-- Built-in: Typed confirmation OR free-text input -->
          <div v-if="content?.requireTyped || content?.inputLabel" class="tokban-modal-typed-section">
            <label
              class="tokban-modal-typed-label"
              v-html="content.requireTyped ? (content?.typedLabel || '') : (content?.inputLabel || '')"
            ></label>
            <input
              ref="typedInputRef"
              type="text"
              ...
              :class="{
                'is-valid':   content?.requireTyped && typedIsValid,
                'is-invalid': content?.requireTyped && typedValue && !typedIsValid,
              }"
              :placeholder="content?.requireTyped
                ? (content?.typedPlaceholder || 'Type to confirm')
                : (content?.inputPlaceholder || '')"
              :value="typedValue"
              @input="handleTypedInput"
            />
            <!-- Hint only shown in confirmation mode -->
            <p v-if="content?.requireTyped && typedValue && !typedIsValid" class="tokban-modal-typed-hint">
              {{ typedHintText }}
            </p>
            <!-- Help text only shown in free-text mode -->
            <p v-if="!content?.requireTyped && content?.inputHelp" class="tokban-modal-input-help">
              {{ content.inputHelp }}
            </p>
          </div>
        </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const confirmEnabled = computed(() => {
      ...
      const hasInput = props.content?.inputLabel;
      if (required) return (typedValue.value || '').trim() === required;
      if (hasInput)  return (typedValue.value || '').trim().length > 0;
      return true;
    });
    ...
    const openModal = () => {
      ...
      nextTick(() => {
        ...
        if ((props.content?.requireTyped || props.content?.inputLabel) && typedInputRef.value) {
