...
export default {
  ...
  setup(props, { emit }) {
    ...
    // ── Open / close ──
    const openModal = () => {
      showModal.value = true; isClosing.value = false;
      setTypedValue(''); setIsLoading(false); setIsOpen(true);
      nextTick(() => {
    ...
    const closeModal = (reason) => {
      ...
      isClosing.value = true; backdropActive.value = false;
      setTimeout(() => {
        showModal.value = false; isClosing.value = false;
        setIsOpen(false); setTypedValue(''); setIsLoading(false);
    ...
    const handleConfirm = () => {
      ...
      emit('trigger-event', { name: 'confirm', event: { action: props.content?.onConfirmAction || '', typedValue: typedValue.value || '' } });
    ...
    return {
      showModal, backdropActive, isClosing, typedInputRef,
      cssVars, iconCircleStyle, iconGlyphStyle,
      confirmBtnStyle, infoBannerStyle, warningBannerStyle,
      checklistItems, confirmEnabled,
      typedValue, typedIsValid, typedHintText, isLoading,
      handleBackdropClick, handleCancel, handleConfirm, handleTypedInput,
      /* wwEditor:start */
      isEditing,
      /* wwEditor:end */
    };
...
.tk-typed-input {
  ...
  border-radius: var(--tk-input-radius);
  font-size: 14px; font-family: inherit; color: #2C3131;
